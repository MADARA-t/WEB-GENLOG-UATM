...
const TeachSidebar = () => {
    ...
    useEffect(() => {
        // Récupération des données stockées lors de la connexion
        const savedUser = JSON.parse(localStorage.getItem('user'));
        if (savedUser) {
            setCurrentUser({
                name: savedUser.name,
                role: savedUser.role || "Formateur",
        ...
        <>
            ...
            >
                ...
                <div className="mt-auto pt-6 border-t border-slate-100 flex flex-col gap-4">
                    ...
                    {/* User Profile Summary DYNAMIQUE */}
                    <div className={`flex items-center gap-4 bg-slate-50 rounded-2xl border border-slate-100 transition-all ${isCollapsed ? 'p-1.5 justify-center' : 'p-3'}`}>
                        <div className="relative flex-shrink-0">
                            <div
                                className="bg-center bg-no-repeat bg-cover rounded-full size-10 shadow-sm border-2 border-white bg-slate-200"
                        ...
                        {!isCollapsed && (
                            <div className="flex flex-col min-w-0 flex-1 animate-in fade-in duration-500">
                                ...
                                <p className="text-[#ea580c] text-[12px] font-medium uppercase tracking-wider">
                                    {currentUser.role}
                                </p>
                            </div>
                        )}
                    ...
                    <button 
                        ...
                        className={`flex items-center gap-2 h-12 rounded-xl bg-slate-100 text-slate-900 text-sm font-bold hover:bg-slate-200 transition-all ${isCollapsed ? 'justify-center w-full' : 'px-4 w-full'}`}
                    >
