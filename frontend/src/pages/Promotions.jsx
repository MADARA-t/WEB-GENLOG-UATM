...
const PromotionCard = ({ promo, onClick, onRefresh, type = "active" }) => {
  ...
  const handleDelete = async (e) => {
    e.stopPropagation();
    if (!window.confirm(`Êtes-vous sûr de vouloir supprimer la promotion "${title}" ?`)) return;
    try {
  ...
  return (
    <div onClick={() => onClick(promo)} className={`group relative flex flex-col bg-white rounded-[2rem] p-8 shadow-sm border border-transparent transition-all duration-300 hover:-translate-y-1 cursor-pointer ${isArchived ? 'opacity-75 hover:opacity-100 bg-slate-50/50' : 'hover:shadow-xl hover:shadow-orange-500/10 hover:border-orange-100'}`}>
      <div className="flex justify-between items-start mb-6">
        ...
        <div className="relative">
          <button onClick={(e) => { e.stopPropagation(); setShowOptions(!showOptions); }} className="w-10 h-10 rounded-full flex items-center justify-center text-slate-300 hover:text-slate-600 hover:bg-slate-50 transition-colors">
          ...
          {showOptions && (
            <>
              ...
              <div className="absolute right-0 top-12 w-48 bg-white rounded-2xl shadow-xl border border-slate-100 z-20 overflow-hidden">
                <button onClick={handleDelete} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-red-50 text-red-600 transition-colors">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${status === 'En cours' ? 'bg-green-100 text-green-700' : status === 'Rentrée' ? 'bg-orange-100 text-orange-700' : 'bg-slate-200 text-slate-600'}`}>{status}</span>
      ...
      <div className="mt-auto pt-6 border-t border-slate-100">
        ...
        <div className="flex items-center justify-between">
          <div className="flex -space-x-2">
            {[1, 2, 3].map((i) => (
              <img key={i} src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${title}${i}`} className="w-8 h-8 rounded-full border-2 border-white bg-slate-100" alt="student" />
            ))}
            <div className="w-8 h-8 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-[10px] font-black text-slate-500">+{studentsCount}</div>
          </div>
          <span className="text-orange-500 text-sm font-black uppercase tracking-widest flex items-center gap-1 group-hover:gap-2 transition-all">Détails <span className="material-symbols-outlined text-sm">arrow_forward</span></span>
        </div>
      </div>
    </div>
  );
...
export default function Promotions() {
  ...
  const fetchAvailableStudents = async () => {
    try {
      ...
      const students = response.data.filter(user => user.role === 'etudiant' && user.isActive).map(user => ({
        id: user.id,
  ...
  const totalStudents = useMemo(() => filteredPromotions.reduce((acc, curr) => acc + curr.studentsCount, 0).toLocaleString(), [filteredPromotions]);

  const unassignedStudents = useMemo(() => {
    return availableStudents.filter(student => !cohortStudents.find(cs => cs.id === student.id));
  }, [availableStudents, cohortStudents]);
  ...
  const handleUpdatePromo = async () => {
    try {
      await api.patch(`/promotions/${selectedPromo.id}`, { name: selectedPromo.title });
  ...
  const handleRemoveStudent = async (enrollmentId, studentName) => {
    if (!window.confirm(`Retirer ${studentName} de cette promotion ?`)) return;
    try {
  ...
  const handleMassImport = async () => {
    if (unassignedStudents.length === 0) {
      alert('Tous les étudiants sont déjà inscrits !');
      return;
    }
    if (!window.confirm(`Voulez-vous ajouter ${unassignedStudents.length} étudiant(s) à la promotion "${selectedPromo.title}" ?`)) return;
    try {
      ...
      if (successCount > 0) {
        alert(`✓ ${successCount} étudiant(s) ajouté(s) avec succès !`);
        fetchPromotionStudents(selectedPromo.id);
      }
      if (errorCount > 0) alert(`⚠️ ${errorCount} erreur(s) lors de l'import`);
    } catch (error) {
    ...
    return (
      <div className="flex-1 flex flex-col h-full overflow-y-auto bg-[#f8f7f5] p-6 lg:p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="max-w-6xl mx-auto w-full">
          <button onClick={() => { setSelectedPromo(null); setIsEditing(false); setStudentSearch(""); setCohortStudents([]); }} className="flex items-center gap-2 text-slate-400 font-black uppercase text-[10px] mb-8 hover:text-orange-500 transition-colors">
            ...
            <div className="p-10 border-b border-slate-50 bg-slate-50/30">
              <div className="flex flex-col md:flex-row justify-between gap-8">
                <div className="flex gap-6 items-center">
                  ...
                  <div>
                    {isEditing ? (
                      <input className="text-3xl font-black text-slate-900 uppercase tracking-tighter border-b-2 border-orange-500 focus:outline-none bg-transparent" value={selectedPromo.title} onChange={(e) => setSelectedPromo({ ...selectedPromo, title: e.target.value })} />
                    ) : (
                      <>
                ...
                <div className="flex gap-4">
                  <StatCard label="Inscrits" value={selectedPromo.studentsCount} isPrimary />
                  <StatCard label="Disponibles" value={unassignedStudents.length} />
                </div>
              </div>
            </div>
              ...
              <div className="p-8 border-r border-slate-100 space-y-8">
                <section>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    {isEditing && (<button onClick={handleUpdatePromo} className="text-green-600 material-symbols-outlined text-sm">check_circle</button>)}
                  </div>
                  <div className="space-y-4">
                    <div>
                      <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Campus</p>
                      {isEditing ? (<input className="text-xs font-bold text-slate-900 border-b w-full" value={selectedPromo.campus} onChange={(e) => setSelectedPromo({ ...selectedPromo, campus: e.target.value })} />) : (<p className="text-xs font-bold text-slate-900">{selectedPromo.campus}</p>)}
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Niveau</p>
                      {isEditing ? (<input className="text-xs font-bold text-slate-900 border-b w-full" value={selectedPromo.level} onChange={(e) => setSelectedPromo({ ...selectedPromo, level: e.target.value })} />) : (<p className="text-xs font-bold text-slate-900">{selectedPromo.level}</p>)}
                    </div>
                  </div>
                </section>
              ...
              <div className="lg:col-span-3 p-8">
                  ...
                  <div className="mb-8 p-6 bg-gradient-to-br from-green-50 to-emerald-50 rounded-3xl border-2 border-green-200">
                    <div className="flex items-center justify-between gap-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="text-lg font-black text-slate-900 uppercase mb-1">Import en masse disponible</h3>
                          <p className="text-xs text-slate-600 font-bold">{unassignedStudents.length} étudiant(s) non inscrit(s) peuvent être ajoutés automatiquement</p>
                        </div>
                      </div>
                      <button onClick={handleMassImport} disabled={importLoading} className="bg-green-500 hover:bg-green-600 text-white px-6 py-4 rounded-2xl flex items-center gap-3 transition-all disabled:opacity-50 shadow-lg shadow-green-200 font-black text-sm uppercase">
                        <span className="material-symbols-outlined">bolt</span>
                        {importLoading ? 'Import...' : 'Importer tous'}
                      </button>
                    </div>
                  </div>
                )}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                  ...
                  <div className="flex flex-1 md:justify-end items-center gap-3 w-full md:w-auto">
                    <div className="relative flex-1 md:flex-none md:w-64">
                      ...
                      <input className="w-full pl-10 pr-4 py-2 bg-slate-50 border-none rounded-xl text-xs font-bold placeholder-slate-300 focus:ring-1 focus:ring-orange-500" placeholder="Chercher..." value={studentSearch} onChange={(e) => setStudentSearch(e.target.value)} />
                    </div>
                    <select className="bg-transparent border-none text-[10px] font-black uppercase text-slate-400 focus:ring-0 cursor-pointer" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                      <option value="name">Nom</option>
                      <option value="date">Date</option>
                    </select>
                    <button onClick={() => setIsAddingStudent(!isAddingStudent)} className="bg-orange-500 text-white p-2 rounded-lg flex items-center hover:bg-orange-600 transition-all">
                      <span className="material-symbols-outlined">person_add</span>
                    </button>
                  </div>
                </div>
                {isAddingStudent && (
                  <div className="mb-6 p-4 bg-orange-50 rounded-2xl border border-orange-100 animate-in slide-in-from-top-2">
                    <div className="flex gap-3">
                      <select required value={selectedStudentId} onChange={(e) => setSelectedStudentId(e.target.value)} className="flex-1 bg-white border-none rounded-xl px-4 text-xs font-bold">
                        <option value="">-- Sélectionner un étudiant --</option>
                        {unassignedStudents.map(student => (
                          <option key={student.id} value={student.id}>{student.name} ({student.email})</option>
                        ))}
                      </select>
                      <button onClick={(e) => handleAddStudent(e)} className="bg-orange-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase">Ajouter</button>
                      <button type="button" onClick={() => { setIsAddingStudent(false); setSelectedStudentId(""); }} className="bg-slate-200 text-slate-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase">Annuler</button>
                    </div>
                  </div>
                )}
                <div className="overflow-x-auto">
                      ...
                      {displayedStudents.map((student) => (
                        <tr key={student.id} className="group hover:bg-slate-50/50 transition-all">
                          ...
                          <td className="py-4 text-right">
                            <button onClick={() => handleRemoveStudent(student.enrollmentId, student.name)} className="text-slate-300 hover:text-red-500 transition-colors" title="Retirer de la promotion">
                  ...
                  {displayedStudents.length === 0 && (
                    <div className="py-20 text-center text-slate-300 text-xs font-bold uppercase tracking-widest">Aucun étudiant dans cette promotion</div>
                  )}
                </div>
              </div>
            </div>
    ...
    <div className="flex-1 flex flex-col h-full overflow-y-auto bg-[#f8f7f5] scrollbar-hide">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in duration-300">
            ...
            <div className="space-y-5">
              ...
              <button onClick={handleSubmit} className="w-full bg-orange-500 text-white font-black uppercase py-4 rounded-2xl shadow-xl shadow-orange-200 mt-4 active:scale-95 transition-all">Créer la cohorte</button>
            </div>
          </div>
        </div>
      )}
      <div className="px-8 lg:px-12 py-8 max-w-[1440px] mx-auto w-full">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-end gap-8 mb-12">
          <div>
            <h1 className="text-slate-900 text-5xl font-black tracking-tighter leading-none mb-6 uppercase">Gestion des <br /><span className="text-orange-500">Promotions</span></h1>
            <p className="text-slate-500 text-lg font-medium max-w-xl">Administrez les cohortes, gérez les inscriptions et suivez la progression académique.</p>
          </div>
          <div className="flex gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mb-12">
          {filteredPromotions.map((promo) => (
            <PromotionCard key={promo.id} promo={promo} onClick={(p) => setSelectedPromo(p)} onRefresh={fetchPromotions} />
          ))}
