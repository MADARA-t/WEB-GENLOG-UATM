...
const EnrollmentManagement = () => {
  ...
  // --- DONNÉES (Initialisées vides, chargées depuis Supabase) ---
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const { data: promoData, error: pError } = await supabase.from('promotions').select('*');
      const { data: userData, error: uError } = await supabase.from('users').select('*');
  ...
  const handleRelance = async (id) => {
    try {
      ...
      const { error } = await supabase.from('users').update({ last_relance: today }).eq('id', id);
      if (error) throw error;
...
// --- SOUS-COMPOSANTS (FORMULAIRES AVEC LOGIQUE SUPABASE) ---
