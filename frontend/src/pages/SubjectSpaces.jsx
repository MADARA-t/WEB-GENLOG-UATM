...
const SubjectSpaces = () => {
  ...
  const fetchInitialData = async () => {
    try {
      const { data: subjectData, error: sError } = await supabase
        .from('subjects')
        .select(`
          *,
      ...
      const { data: promoData } = await supabase.from('promotions').select('name, year');
      if (promoData) setAvailablePromotions(promoData);

      const { data: usersData } = await supabase.from('users').select('id, name, role').order('name');
      if (usersData) {
  ...
  const addStudentManually = async (name) => {
    if (!name.trim()) return;
    try {
      const { data: userData, error: uError } = await supabase.from('users').select('id').eq('name', name).eq('role', 'Étudiant').single();
      ...
      const { error: eError } = await supabase.from('subject_enrollments').insert([{
  ...
  const removeStudent = async (studentName) => {
    try {
      const { data: userData } = await supabase.from('users').select('id').eq('name', studentName).single();
      if (userData) {
        const { error } = await supabase.from('subject_enrollments')
          .delete()
  ...
  const assignInstructor = async (name) => {
    try {
      ...
      if (name) {
        const { data: userData } = await supabase.from('users').select('id').eq('name', name).eq('role', 'Formateur').single();
      ...
      const { error } = await supabase.from('subjects').update({ instructor_id: instructorId }).eq('id', selectedSubject.id);
      if (error) throw error;
